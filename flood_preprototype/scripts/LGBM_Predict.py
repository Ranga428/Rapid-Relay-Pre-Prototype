...
Runs the deployed XGBoost sensor model (flood_lgbm_sensor.pkl).
No satellite data required at inference time.

Outputs (written to ../predictions/):
    flood_lgbm_sensor_predictions.csv
    flood_lgbm_sensor_predictions.png
...
# LGBM artifacts store a val-tuned threshold saved by train_flood_model_lgbm.py.
...
def load_model() -> tuple:
    ...
    if not os.path.exists(MODEL_FILE):
        sys.exit(
            ...
            f"  Fix      : Run train_flood_model_lgbm.py first.\n"
        )
...
def filter_new_rows(features: pd.DataFrame) -> pd.DataFrame:
    ...
    if len(new_df) == 0:
        sys.exit(
            ...
            f"  LAST_TRAINING_DATE in predict_lgbm.py."
        )
...
def run_pipeline() -> None:
    separator("Flood Prediction — LightGBM Sensor Inference Pipeline")
