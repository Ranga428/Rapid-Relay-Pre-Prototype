...
Runs the deployed Random Forest sensor model (flood_rf_sensor.pkl).
No satellite data required at inference time.

Design option  : Random Forest (Design Option 2 of 3)
Artifact       : flood_rf_sensor.pkl
Script         : predict_rf.py

Usage
-----
    python predict_rf.py
    python predict_rf.py --data ..\\data\\flood_dataset_test.csv
    python predict_rf.py --schedule --interval 12
    python predict_rf.py --data ..\\data\\flood_dataset_test.csv --threshold-search
...
def load_model() -> tuple:
    ...
    # Read last_training_date from artifact (saved by RF_train_flood_model.py).
    # Falls back to the LAST_TRAINING_DATE constant if not present in older pkls.
...
def filter_new_rows(features: pd.DataFrame) -> pd.DataFrame:
    ...
    if len(new_df) == 0:
        sys.exit(
            ...
            f"  Update LAST_TRAINING_DATE in predict_rf.py after retraining."
        )
...
def run_threshold_search(model, feature_cols, features, ground_truth) -> None:
    ...
    print(f"    --model ..\\model\\flood_rf_sensor.pkl")
