...
MODEL_FILE       = os.path.join(SCRIPT_DIR, "..", "model",       "flood_xgb_sensor.pkl")
...
def run_pipeline(data_path=None, threshold_search=False) -> None:
    ...
    if batch_mode:
        ...
        output_csv  = os.path.join(OUTPUT_DIR, f"flood_xgb_{stem}_predictions.csv")
        output_plot = os.path.join(OUTPUT_DIR, f"flood_xgb_{stem}_predictions.png")
    else:
